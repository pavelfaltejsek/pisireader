...
namespace GecoSI.Net.Internal
{
    public class SiMessage
    {
        ...
        public const byte GET_TIME = 0xF7;
        ...
        public static readonly SiMessage GET_READOUT_TIME = new SiMessage(new byte[]
        {
            //SendCommand(stx,cmd,delka pars crc16 etx  bez pars je to jen opakovani)

            STX, GET_TIME, 0x00, GET_TIME, 0x00, ETX
        });
        ...
        public DateTime getTime()
        {
            if (ValidCrc())
            {
                ...
                long secs = ((long)(bintime[8]) << 8) + (long)(bintime[9]);
                int h = (int)(secs / 3600);
                int m = (int)(secs % 3600) / 60;
                int s = (int)(secs % 60);

                int ampm = bintime[7] & 1;
                if (ampm > 0) h += 12;
                DateTime rt = new DateTime(2000 + bintime[4], bintime[5], bintime[6], h, m, s);
                return rt;
            }
            return new DateTime();
        }
