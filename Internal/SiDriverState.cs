...
namespace GecoSI.Net
{
    public class SiDriverState
    {
        ...
        public static readonly SiDriverState GET_READOUT_TIME = new GetReadOutTimeState();
        ...
        public virtual ISiDataFrame CreateDataFrame(SiMessage[] dataMessages, DateTime readout)
        {
            WrongCall();
            return null;
        }
            ...
            SiMessage[] readoutCommands, int nbPunchesIndex, String timeoutMessage)
        {
            try
            {
                ...
                //zkusme zjistit cas
                writer.Write(SiMessage.GET_READOUT_TIME);
                SiMessage tm = PollAnswer(queue, SiMessage.GET_READOUT_TIME.CommandByte());

                siHandler.Notify(CreateDataFrame(dataMessages, tm.getTime()));
        ...
        protected class AckReadDriverState : SiDriverState
        {
            public override SiDriverState Send(ICommWriter writer, SiHandler siHandler)
            {
                ...
                //zkusime zjistit cas a pockat na vytazeni
                //return GET_READOUT_TIME;
            }
        }

        protected class GetReadOutTimeState : SiDriverState
        {
            public override SiDriverState Send(ICommWriter writer, SiHandler siHandler)
            {
                writer.Write(SiMessage.GET_READOUT_TIME);
        ...
        protected class DispatchReadyDriverState : SiDriverState
        {
            public override SiDriverState Receive(SiMessageQueue queue, ICommWriter writer, SiHandler siHandler)
            {
                ...
                switch (message.CommandByte())
                {
                    ...
                    case SiMessage.GET_TIME:
                        //time nastav
                        DateTime rt = message.getTime();
                        GecoSiLogger.Debug("GETTIME " + message + " / " + rt);
                        break;
        ...
        protected class RetrieveSicard10PlusDataDriverState : SiDriverState
        {
            ...
            public override ISiDataFrame CreateDataFrame(SiMessage[] dataMessages, DateTime readout)
            {
                return new Si8PlusDataFrame(dataMessages, readout);
            }
        }
        ...
        protected class RetrieveSicard5DataDriverState : SiDriverState
        {
            ...
            public override ISiDataFrame CreateDataFrame(SiMessage[] dataMessages, DateTime readout)
            {
                return new Si5DataFrame(dataMessages[0], readout);
            }
        }
        ...
        protected class RetrieveSicard6DataDriverState : SiDriverState
        {
            ...
            public override ISiDataFrame CreateDataFrame(SiMessage[] dataMessages, DateTime readout)
            {
                return new Si6DataFrame(dataMessages, readout);
            }
        }
        ...
        protected class StartupCompleteDriverState : SiDriverState
        {
            public override SiDriverState Send(ICommWriter writer, SiHandler siHandler)
            {
                ...
                Thread.Sleep(2000);
                return GET_READOUT_TIME.Send(writer, siHandler);
            }
        }
