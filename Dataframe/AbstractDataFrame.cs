...
namespace PISI.Net.Dataframe
{
    public abstract class AbstractDataFrame : ByteFrame, ISiDataFrame
    {
        ...
        public void PrintString()
        {
            ...
            Console.WriteLine(" - Readout: {0})", FormatTime(ReadOutTime));
        ...
        public string GetString()
        {
            //tohle se tiskne
            string r = "";
            r += String.Format("\n{0}: {1} ", SiSeries, SiNumber);
            if (RunnerName != "") r += String.Format("\n   {0}", RunnerName);
            if (CourseName != "") r += String.Format("\n   {0}", CourseName);
            r += String.Format("\nStart:            {0}", FormatTime(StartTime));
            r += String.Format("\nFinish:           {0}", FormatTime(FinishTime));
            r += String.Format("\nCheck:            {0}", FormatTime(CheckTime));
            r += String.Format("\nReadOut:          {0}", FormatTime(ReadOutTime));
            r += String.Format("\nTime:               {0}", FormatMin(FinishTime - StartTime));
            r += String.Format("\nPunches: {0}", NbPunches);
            long LastTime = StartTime;
            for (int i = 0; i < NbPunches; i++)
            {
                r += String.Format("\n{0}: {1} {2} - {3}", (i + 1).ToString().PadLeft(3), Punches[i].Code.ToString().PadLeft(3), FormatMin(Punches[i].Timestamp - StartTime), FormatMin(Punches[i].Timestamp - LastTime));
                LastTime = Punches[i].Timestamp;
            }
            r += String.Format("\n{0}: {1} {2} - {3}", "  F", "   ", FormatMin(FinishTime - StartTime), FormatMin(FinishTime - LastTime));
            r += String.Format("\n");
            r += String.Format("\n");
            r += String.Format("\n");
            return r;
        ...
        {
            String DATETIME_FORMATTER = "YYYY-MM-DD H:mm:ss";
            if (timestamp == NO_TIME)
            {
                return "no time";
            }
            var dtDateTime = new DateTime(1970, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc);
            dtDateTime = dtDateTime.AddMilliseconds(timestamp);
            return dtDateTime.ToString(DATETIME_FORMATTER);
        }

        public String FormatMin(long timestamp)
        {
            if (timestamp == NO_TIME)
            {
                return "no time";
            ...
            var dtDateTime = new DateTime(1970, 1, 1, 0, 0, 0, 0, DateTimeKind.Utc);
            dtDateTime = dtDateTime.AddMilliseconds(timestamp);
            int min = dtDateTime.Minute + 60 * dtDateTime.Hour;
            return min.ToString().PadLeft(3) + ":" + dtDateTime.Second.ToString().PadLeft(2, '0');
            ;
        }
    }
}